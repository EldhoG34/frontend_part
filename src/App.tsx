...
const SERVER_URL = 'https://backndtest.onrender.com';
...
function App() {
  ...
  useEffect(() => {
    ...
    const handleExecutionResult = ({ filePath, output }: { filePath: string, output: string }) => {
      // Optionally check if filePath matches currentFile
      if (filePath === currentFile) {
        setOutput(output);
  ...
  const executeCode = () => {
    ...
    // Emit the execute-code event with room, current file, and current editor content
